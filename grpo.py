...
class DiscretePolicy:
    ...
    def __init__(self, num_states: int, num_actions: int, seed: int = 42):
        ...
        self.logits = [
            [self.rng.uniform(-0.01, 0.01) for _ in range(num_actions)]
    ...
    def grpo_update(
        ...
        for item in batch:
            ...
            # use clipped ratio for update when unclipped is overly optimistic
            weight = ratio if unclipped <= clipped_obj else clipped_ratio
            ...
            for k in range(self.num_actions):
                grad[item.state][k] += entropy_coef * (
                    -probs[k] * (math.log(max(probs[k], EPS)) + h)
                )
...
class ContextualBanditEnv:
    ...
    def sample_state(self) -> int:
        return self.rng.randrange(self.num_states)
...
def collect_grouped_samples(
    ...
    for _ in range(groups_per_epoch):
        ...
        for action, old_logp, reward, adv in zip(
            group_actions, group_old_logps, group_rewards, advs
        ):
            samples.append(
                Sample(
                    state=state,
...
def train_grpo(
    ...
    env = ContextualBanditEnv(num_states=num_states, num_actions=num_actions, seed=seed + 1)
    policy = DiscretePolicy(num_states=num_states, num_actions=num_actions, seed=seed)

    avg_rewards: List[float] = []
    for _ in range(epochs):
        batch = collect_grouped_samples(
            env, policy, group_size=group_size, groups_per_epoch=groups_per_epoch
        )
...
if __name__ == "__main__":
    ...
    final_eval = evaluate_policy(policy, num_states=10, num_actions=3)
    print(f"initial avg reward: {rewards[0]:.3f}")
    print(f"final avg reward:   {rewards[-1]:.3f}")
    print(f"greedy eval reward: {final_eval:.3f}")
